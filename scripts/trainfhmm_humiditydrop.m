humiditydrops = getdatabetweenintervals(data.preprocessed,intervals.IncubatorOpen,obschannels,settings.train.babies);

% get the mean and diagonal covariance of training data
alltrainingdata =cellconcat(humiditydrops);
...
fhmm_trans = markovtransitionprobsfromintervals(intervals.IncubatorOpen,settings.train.babies);
...
fhmm_factor = {fhmm_mu fhmm_cov settings.factors.handling.ydims fhmm_prior};
